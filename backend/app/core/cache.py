...
def cached(ttl_seconds: int = 300, prefix: str = "query"):
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            # Check in-memory fallback cache first if entry valid
            now = asyncio.get_event_loop().time()
            if cache_key in _MEMORY_CACHE:
                exp, val = _MEMORY_CACHE[cache_key]
                if now < exp:
            ...
            try:
                ...
                logger.debug("redis_get_connection_error", error=str(e))
            ...
            if redis:
                try:
                    ...
                    if cached_val is not None:
                        ...
                        _MEMORY_CACHE[cache_key] = (now + ttl_seconds, res)
                        return res
                except Exception as e:
            ...
            if result is not None:
                _MEMORY_CACHE[cache_key] = (now + ttl_seconds, result)
